...
declare module "mongoose" {
	...
	> = DocType extends Document
		...
		: Document<unknown, any, DocType> &
				MergeType<Require_id<DocType>, TVirtuals & TMethodsAndOverrides>;
	...
	>
		? Schema<
				Omit<DocType, keyof DisSchemaEDocType> & DisSchemaEDocType,
	...
	export class Schema<
		...
		DocType extends ApplySchemaOptions<
			ObtainDocumentType<DocType, EnforcedDocType, TSchemaOptions>,
			TSchemaOptions
		> = ApplySchemaOptions<
			ObtainDocumentType<any, EnforcedDocType, TSchemaOptions>,
			TSchemaOptions
		>
	> extends events.EventEmitter {
		...
		constructor(
			definition?:
				| SchemaDefinition<SchemaDefinitionType<EnforcedDocType>>
				| DocType,
			options?:
				| SchemaOptions<
						FlatRecord<DocType>,
		...
		add(
			obj: SchemaDefinition<SchemaDefinitionType<EnforcedDocType>> | Schema,
			prefix?: string
		): this;
		...
		path<
			ResultType extends SchemaType = SchemaType<
				any,
				HydratedDocument<DocType, TInstanceMethods>
			>
		>(path: string): ResultType;
		path<pathGeneric extends keyof EnforcedDocType>(
			path: pathGeneric
		...
		plugin<
			PFunc extends PluginFunction<DocType, M, any, any, any, any>,
		...
		static(
			obj: { [F in keyof TStaticMethods]: TStaticMethods[F] } & {
				[name: string]: (this: M, ...args: any[]) => any;
			}
		): this;
		...
		virtualpath<T = HydratedDocument<DocType, TInstanceMethods>>(
			name: string
		): VirtualType<T> | null;
	}
	...
	export type SchemaDefinitionProperty<T = undefined> =
	...
	export interface MixedSchemaTypeOptions
		extends SchemaTypeOptions<Schema.Types.Mixed> {
	...
	export type LeanType<T> = 0 extends 1 & T
		? T // any
		: T extends TreatAsPrimitives
		? T // primitives
		: T extends Types.ArraySubdocument
		? Omit<LeanDocument<T>, "parentArray" | "ownerDocument" | "parent">
		: T extends Types.Subdocument
		? Omit<LeanDocument<T>, "$isSingleNested" | "ownerDocument" | "parent">
		: LeanDocument<T>; // Documents and everything else

	export type LeanArray<T extends unknown[]> = T extends unknown[][]
		? LeanArray<T[number]>[]
		: LeanType<T[number]>[];
	...
	export type LeanDocumentElement<T> = T extends unknown[]
		? LeanArray<T> // Array
		: T extends Document
	...
	export type DocTypeFromUnion<T> = T extends Document<
		infer T1,
		infer T2,
		infer T3
	> &
		infer U
		? [U] extends [Document<T1, T2, T3> & infer U]
			? IfUnknown<IfAny<U, false>, false>
			: false
		: false;

	export type DocTypeFromGeneric<T> = T extends Document<
		infer IdType,
		infer TQueryHelpers,
		infer DocType
	>
		? IfUnknown<IfAny<DocType, false>, false>
		: false;
	 ...
	 */
	export type BaseDocumentType<T> = _pickObject<
		DocTypeFromUnion<T>,
		DocTypeFromGeneric<T>,
		false
	>;
